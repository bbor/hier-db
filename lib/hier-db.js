...
db.prototype.generate_uid = function(name, types){
  ...
  var uid = alphafy(name).toLowerCase();
  if (!this.index[uid]) {
    return uid;
  }
  if (types) {
...
db.prototype.add = function(records, parent)
{
  ...
  for (var i_record = 0; i_record < records.length; i_record++)
  {
    ...
    assert(record[this.config.name], 'This record needs a name! ' + JSON.stringify(record));
    ...
    if (!parent && (!record[this.config.parents] || record[this.config.parents].length == 0))
    {
      record[this.config.parents] = ['root'];
    }
    if (_.includes(record[this.config.parents], 'root'))
    {
      this.add_parent(record, 'root');
    }
  }
}
